...
;
PGZERO_ST       =    $D0             ;Start of Page Zero usage
;
;Buffer used by the default Monitor code
INBUFF          =    PGZERO_ST+0     ;4 byte buffer for HEX input ($D0-$D3)
;
;16-bit variables
HEXDATAH        =    PGZERO_ST+4     ;Hexadecimal input
HEXDATAL        =    PGZERO_ST+5
BUFADRL         =    PGZERO_ST+6     ;Input address
BUFADRH         =    PGZERO_ST+7
COMLO           =    PGZERO_ST+8     ;User command address
COMHI           =    PGZERO_ST+9
INDEXL          =    PGZERO_ST+10    ;Index for address
INDEXH          =    PGZERO_ST+11
TEMPL           =    PGZERO_ST+12    ;Index for word temp value used by Memdump
TEMPH           =    PGZERO_ST+13
PROMPTL         =    PGZERO_ST+14    ;Prompt string address
PROMPTH         =    PGZERO_ST+15
SRCL            =    PGZERO_ST+16    ;Source address for memory operations
SRCH            =    PGZERO_ST+17
TGTL            =    PGZERO_ST+18    ;Target address for memory operations
TGTH            =    PGZERO_ST+19
LENL            =    PGZERO_ST+20    ;Length address for memory operations
LENH            =    PGZERO_ST+21
;
;8-bit variables and constants:
BUFIDX          =    PGZERO_ST+22    ;Buffer index
BUFLEN          =    PGZERO_ST+23    ;Buffer length
INDEX           =    PGZERO_ST+24    ;Temp Indexing
TEMP1           =    PGZERO_ST+25    ;Temp value - Code Conversion routines
TEMP2           =    PGZERO_ST+26    ;Temp value - Memory routines
CMDFLAG         =    PGZERO_ST+27    ;Command Flag - used by RDLINE
SPAREM1         =    PGZERO_ST+28    ;Spare byte 1
SPAREM2         =    PGZERO_ST+29    ;Spare byte 2
SPAREM3         =    PGZERO_ST+30    ;Spare byte 3
SPAREM4         =    PGZERO_ST+31    ;Spare byte 4
;
;BIOS variables, pointers, flags located at top of Page Zero.
BIOS_PG0        =    PGZERO_ST+32    ;Start of BIOS page zero use
;
;BRK handler routine
...
SPAREB1         =    BIOS_PG0+14     ;Spare byte 1
SPAREB2         =    BIOS_PG0+15     ;Spare byte 2
;
;**************************************************************************************************
...
;
NMIVEC0         =    SOFTVEC         ;NMI Interrupt Vector
BRKVEC0         =    SOFTVEC+2       ;BRK Interrupt Vector
IRQVEC0         =    SOFTVEC+4       ;INTERRUPT VECTOR
;
NMIRTVEC0       =    SOFTVEC+6       ;NMI Return Handler
BRKRTVEC0       =    SOFTVEC+8       ;BRK Return Handler
IRQRTVEC0       =    SOFTVEC+10      ;IRQ Return Handler
;
CLDMNVEC0       =    SOFTVEC+12      ;Cold Monitor Entry Vector
WRMMNVEC0       =    SOFTVEC+14      ;Warm Monitor Entry Vector
;
VECINSRT0       =    SOFTVEC+16      ;1st Vector Insert
VECINSRT1       =    SOFTVEC+18      ;2nd Vector Insert
VECINSRT2       =    SOFTVEC+20      ;3rd Vector Insert
VECINSRT3       =    SOFTVEC+22      ;4th Vector Insert
;
;**************************************************************************************************
SOFTCFG         = SOFTVEC+24         ;Start of hardware config parameters
...
SRCHBUFF        =    $330            ;Located in Page $03 following HW config data
;
;ACIA device address
...
;**************************************************************************************************
                .segment "CODE"           ;Target address range $F800 through $FDFF will be used
;**************************************************************************************************
...
;
MONTAB:         .addr     RESET           ;[CNTL-R] $12 Reset
                ...
                .addr     CPMVFL          ;C $43 Compare memory command
                ...
                .addr     CPMVFL          ;M $4D Copy memory range to a specified target address
                .addr     PRG             ;P $50 Examine/change PROC STATUS REGISTER preset/result
...
;START OF TOP PAGE - DO NOT MOVE FROM THIS ADDRESS!!
                .segment "OS"           ;JMP Table, HW Vectors, Cold Init and Vector handlers
...
;
VEC_TABLE:      .addr     NMI_VECTOR      ;NMI Location in ROM
                .addr     BRKINSTR0       ;BRK Location in ROM
                .addr     INTERUPT0       ;IRQ Location in ROM
                .addr     NMIHNDLR0       ;NMI return handler in ROM
                .addr     IRQ_EXIT0       ;BRK return handler in ROM
                .addr     IRQ_EXIT0       ;IRQ return handler in ROM
                .addr     MONITOR         ;Monitor Cold start
                .addr     NMON            ;Monitor Warm start
;
;Vector Inserts (total of 4)
; as NMI/BRK/IRQ and the Monitor are vectored, all can be extended
; by using these reserved vectors.
                .addr     $FFFF           ;Insert 0 Location
                .addr     $FFFF           ;Insert 1 Location
                .addr     $FFFF           ;Insert 2 Location
                .addr     $FFFF           ;Insert 3 Location
;
CFG_TABLE:      ;Configuration table for hardware devices
;
CFG_6551:       .byte     $09             ;Default 65C51 Cmd reg, Rcv IRQ enabled
                .byte     $1F             ;Default 65C51 Ctl reg, (19.2K, 8-N-1)
;
...
BIOS_MSG:       .byte     $0D,$0A         ;BIOS startup message
                .byte     "65C02 "
                .byte     "BIOS V1.2L"
                .byte     $0D,$0A         ;Add CR/LF
                .byte     $00            ;Terminate string
;
                .segment  "VECTORS"       ;65C02 Hardware Vectors
                .addr     NMIVEC0         ;NMI
                .addr     COLDSTRT        ;RESET
                .addr     IRQ_VECTOR      ;IRQ
;**************************************************************************************************
